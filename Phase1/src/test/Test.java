...
public class Test {
	...
	private JLabel LUText, StoppingConditionText, IterationsNumberText, ErrorText;
	private JComboBox LUComboBox, StoppingConditionComboBox;
	private JSpinner IterationsNumberSpinner;
	private JButton button;
	int x = 20, y = 280, width = 110, height = 35;
	...
	private void initialize() {
		...
		for(int i = 0; i < 10; i++) {
			...
			for(int j = 0; j < 11; j++) {
				...
				coef[i][j].setFont(new Font("Tahoma", Font.PLAIN, 26));
				coef[i][j].setText("0");
				...
				if(j != 10) {
					...
					labels[i][j].setBounds(x + 115, y, 70, height);
		...
		button.setBounds(1756, 11, 138, 63);
	...
	private void solve(String method, int n) {
		...
		Solve obj = new Solve(method, mat);
